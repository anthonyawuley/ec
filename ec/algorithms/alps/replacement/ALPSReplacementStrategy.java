...
public class ALPSReplacementStrategy {
	...
	public Population interLayerMigrations(ArrayList<Layer> layers,Population move,int to)
	{
		...
		//re-evaluate ages of population
		 ...
		 {
			 move.calculateAge(Engine.completeEvaluationCount,layers.get(to).getParameters().getPopulationSize());
			 higherPop.calculateAge(Engine.completeEvaluationCount,layers.get(to).getParameters().getPopulationSize());	
		 }
